if (typeof(Dr) === 'undefined') {
    ...
    Dr.document.addEventListener("keydown", function (event) {
        Dr.log("[KEY_DOWN]", event.keyCode, event.which, Dr.getKeyDefinition(event.keyCode || event.which));
        Dr.Event.emit("KEY_DOWN", event, Dr.getKeyDefinition(event.keyCode || event.which));
    });
    Dr.document.addEventListener("keyup", function (event) {
        Dr.log("[KEY_UP]", event.keyCode, event.which, Dr.getKeyDefinition(event.keyCode || event.which));
        Dr.Event.emit("KEY_UP", event, Dr.getKeyDefinition(event.keyCode || event.which));
    });
    Dr.document.addEventListener("keypress", function (event) {
        Dr.log("[KEY_PRESS]", event.keyCode, event.which, Dr.getKeyDefinition(event.keyCode || event.which));
    ...
    Dr.afterWindowLoaded = function (callback) {
        if (Dr.WINDOW_LOADED) {
            ...
            Dr.Event.addEventListener('WINDOW_LOADED', callback);
        }
    };
    ...
    Dr.simulateClientClick = (function () {
        ...
        else if (document.createEventObject) { //For IE 8
            return function (type, element, view, client_x, client_y) {
    ...
    Dr.loadImage = function (image_src, callback) {
        ...
        image_element.addEventListener('load', function () {
    ...
    Dr.createHttpRequest = function (url, message, finish_callback, state_change_callback) {
        ...
        xml_http.addEventListener('readystatechange', function () {
    ...
    Dr.setStyleTransformDegree = (function () {
        ...
        return function (element, rotate_degree) {
            if (!key_transform) {
                ...
                if (!key_transform) {
                    alert('style unsupported transform');
                    key_transform = 'unsupported_transform';
                }
            }
    ...
    Dr.parseFile = function (file, parse_method, callback) {
        ...
        switch (parse_method) {
            ...
            case 'text':
                ...
                break;
            default:
                alert('[parseFile] error parse_method');
                break;
        }
    }
    ...
    //event related
    Dr.getKeyDefinition = function (key_code) {
    ...
    Dr.Implement('FPS', function (global, module_get) {
        var Module = function (output_func) {
            //record output func, so you don't need to set it next time
