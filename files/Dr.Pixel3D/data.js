...
Dr.Declare('Pixel3D_Data', 'class');
Dr.Require('Pixel3D_Data', 'Pixel3D_Math');
Dr.Implement('Pixel3D_Data', function (global, module_get) {
	var Module = Module || {};
	...
	var Vertex = (function () {
		...
		Vertex.Zero = function () {
			return new Vertex(Vector3.Zero());
		}
	...
	var Light = (function () {
		...
		Light.FromJSON = function (jsonLight) {
			...
			//Load light
			return new Light(
				Vector3.FromArray(jsonLight.position),
				Color4.FromArray(jsonLight.color),
				jsonLight.name
			);
		}
		return Light;
	...
	var Camera = (function () {
		...
		Camera.prototype.rotatePosition = function (rX, rY, rZ) {
			...
			var transMatrix = Matrix.RotationYawPitchRoll(rY, rX, rZ);
			forwardVector = Vector3.TransformCoordinates(
		...
		BlockMesh.FromJSON = function (jsonMesh) {
			...
			for (var indexBlock = 0; indexBlock < jsonMesh.points.length; indexBlock++) {
				...
				tempMesh.Blocks[indexBlock] = new Block(
					Vector3.FromArray(jsonMesh.points[indexBlock][0]),
					Color4.FromArray(
						jsonMesh.colorList[jsonMesh.points[indexBlock][1]]
					)
				);
			}
			tempMesh.Position = Vector3.FromArray(jsonMesh.position);
			tempMesh.Rotation = Vector3.FromArray(jsonMesh.rotation);
			//important move!!
			tempMesh.removeInvisible();
			return tempMesh;
		}
	...
	*/
	...
	var ModelBlock = (function () {
		function ModelBlock() {
			//for pre-transform
			this.VisibleFaces = [];	//after tranform we'll find 3 max
		...
		if (!ModelBlock.CenterVertex) {
			ModelBlock.CenterVertex = new Vertex(new Vector3(0, 0, 0));
		}
		
		if (!ModelBlock.Vertices) {
			ModelBlock.Vertices = new Array(8);
			//size should be 1 x 1 x 1, scaled it a bit to prevent void gap
			ModelBlock.Vertices[0] = new Vertex(new Vector3(0.500000, 0.500000, 0.500000));
			ModelBlock.Vertices[1] = new Vertex(new Vector3(0.500000, 0.500000, -0.500000));
			ModelBlock.Vertices[2] = new Vertex(new Vector3(0.500000, -0.500000, 0.500000));
			ModelBlock.Vertices[3] = new Vertex(new Vector3(0.500000, -0.500000, -0.500000));
			
			ModelBlock.Vertices[4] = new Vertex(new Vector3(-0.500000, 0.500000, 0.500000));
			ModelBlock.Vertices[5] = new Vertex(new Vector3(-0.500000, 0.500000, -0.500000));
			ModelBlock.Vertices[6] = new Vertex(new Vector3(-0.500000, -0.500000, 0.500000));
			ModelBlock.Vertices[7] = new Vertex(new Vector3(-0.500000, -0.500000, -0.500000));
		}
		if (!ModelBlock.Faces) {
			...
			//all unit Vector
			ModelBlock.Faces[0].Normal = new Vector3(1, 0, 0);
			ModelBlock.Faces[1].Normal = new Vector3(0, 1, 0);
			ModelBlock.Faces[2].Normal = new Vector3(0, 0, 1);
			ModelBlock.Faces[3].Normal = new Vector3(-1, 0, 0);
			ModelBlock.Faces[4].Normal = new Vector3(0, -1, 0);
			ModelBlock.Faces[5].Normal = new Vector3(0, 0, -1);
		}
		...
		ModelBlock.prototype.preTransform = function (device, transformMatrix, globalLightPack, worldMatrix, worldViewMatrix, optionExtra) {
			...
			this.TransSize = Math.max(tempVector1.length(), tempVector2.length()) / 2;
																				//Dr.log("TransSize:"+this.TransSize);
			...
			for (var indexFace = 0; indexFace < this.VisibleFaces.length; indexFace++) {
				...
				*/
				...
				var worldNormal = Vector3.TransformNormal(currentFace.Normal, worldMatrix);
				this.WorldNormals[indexFace] = worldNormal;
				
				///calc world Global lights & world normal
				var finalGlabalLight = new Color4(0, 0, 0, 0); //black & transparent
				//for each light:
				for (var indexLight = 0; indexLight < globalLightPack.length; indexLight++) {
					...
					//Blending
					finalGlabalLight = Color4.MethodBlend(
						finalGlabalLight,
