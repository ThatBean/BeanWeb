...
function fileMod() {
	$file_name="buylist_" . $_POST["file_name"];
	$file_path=".";
...
function fileAdd() {
	...
	$file = fopen($file_path,"wb");
	if ($file) {
		$response=$response . "[mod]" . fwrite($file,file_get_contents($file_path) . $file_text) . " char written to file. <br />";
		fclose($file);
		$response=$response . "[mod]chOwn/chMod: " . chown($file_path,"Bean") ."/". chmod($file_path,0644) ."<br />";
	}
	else {
...
function fileOpen($msg_sp) {
	...
	//output the response
	echo "[open]get file contents success..." . $msg_sp . $response;
	return;
}
?>